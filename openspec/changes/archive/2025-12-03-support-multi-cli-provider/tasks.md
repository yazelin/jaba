...
### Gemini Session 管理
- [ ] 實作 Gemini session 索引追蹤（解析 `--list-sessions` 或追蹤新建 session）
- [ ] 實作 Gemini 後續對話的 `--resume <index>` 邏輯
- [ ] 實作 Gemini session 重置邏輯

### 驗證
- [ ] 測試 Gemini provider 對話功能
- [ ] 測試 Gemini provider 菜單辨識功能
- [ ] 測試混合使用（chat 用 Claude、menu_recognition 用 Gemini）
- [ ] 測試多使用者同時使用 Gemini 的 session 隔離
